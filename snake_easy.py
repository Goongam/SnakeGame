...
랭킹저장: F1'''%(len(snake),surviveTime()),
        ...
        displayScore.write("SCORE:"+str(len(snake)),font=("Arial", 10))
...
def onF1():
    if gameOver:
        turtle.bye()
        Ranking.showRanking(snake,"EASY")
...
def start():
    ...
    screen.bgcolor(option.background_color)
    screen.setup(w, h)
    screen.tracer(0)
    ...
    global pen
    pen = turtle.Turtle(option.snake_shape)
    pen.color(option.snake_color)
    pen.penup() 
